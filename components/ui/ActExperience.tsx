...
export function ActExperience({ productTitle, acts }: ActExperienceProps) {
  ...
  const [activeActId, setActiveActId] = useState<string | null>(autoUnlocked[0] || null);
  const [actFeedback, setActFeedback] = useState<Record<string, { status: "success" | "error"; message: string }>>({});
  const [clueFeedback, setClueFeedback] = useState<Record<string, { status: "success" | "error"; message: string }>>({});
  ...
  useEffect(() => {
    ...
    try {
      ...
      setUnlockedActs(new Set([...autoUnlocked, ...(parsed.unlockedActs || [])]));
  ...
  }, [unlockedActs, revealedClues, unlockAnswers, finalNotes, finalShared, activeActId]);
  ...
  const isActCompleted = (act: Act) => {
    ...
    const allRequiredRevealed = requiredClues.every((clue) => revealedClues.has(String(clue.id)));

    return isUnlocked && allRequiredRevealed;
  };

  const nonFinalActs = orderedActs.filter((act) => !act.isFinalStep);
  const allNonFinalCompleted = nonFinalActs.every((act) => isActCompleted(act));
  ...
  const unlockAct = (act: Act) => {
    ...
    if (act.isFinalStep && !allNonFinalCompleted) {
      setActFeedback({
        ...
        [String(act.id)]: {
          status: "error",
          message: "Debes completar los actos anteriores antes de intentar este desenlace.",
        },
      });
      return;
    }
    ...
    if (act.unlockType === "auto") {
      ...
      setActFeedback({ ...actFeedback, [String(act.id)]: { status: "success", message: "Acto abierto automáticamente." } });
      return;
    }
    ...
    if ((act.unlockType === "code" || act.unlockType === "answer") && expectedCode && attempt === expectedCode) {
      ...
      setActFeedback({
        ...actFeedback,
        [String(act.id)]: { status: "success", message: "Respuesta correcta. Puedes acceder al acto." },
      });
      return;
    }
    ...
    if (act.unlockType === "fileSolved" && attempt.length > 2) {
      ...
      setActFeedback({
        ...actFeedback,
        [String(act.id)]: { status: "success", message: "Perfecto. El archivo resuelto desbloquea este acto." },
      });
      return;
    }
    ...
    if (attempt.length > 0) {
      setActFeedback({
        ...actFeedback,
        [String(act.id)]: { status: "error", message: "La pista no encaja. Revisa las evidencias y vuelve a probar." },
      });
    }
  };
  ...
  const revealClue = (clue: Clue, isUnlocked: boolean) => {
    ...
    if (!expected || attempt === expected) {
      ...
      setClueFeedback({
        ...clueFeedback,
        [String(clue.id)]: { status: "success", message: "Bien jugado. Pista desbloqueada." },
      });
      return;
    }
    ...
    if (attempt.length > 0) {
      setClueFeedback({
        ...clueFeedback,
        [String(clue.id)]: { status: "error", message: "Esa respuesta no encaja con la prueba. Intenta con otro dato." },
      });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        {orderedActs.map((act, index) => {
          ...
          return (
            <Card
              key={act.id}
              className={`border border-black/10 shadow-sm ${isActive ? "ring-2 ring-primary" : ""}`}
            >
              <CardHeader
                    ...
                    <div className="rounded-lg border bg-gray-50 p-4 space-y-3">
                      ...
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                        <Input
                          ...
                          className="bg-white"
                          aria-invalid={actFeedback[actId]?.status === "error"}
                        />
                        <Button onClick={() => unlockAct(act)} className="w-full sm:w-auto">Desbloquear</Button>
                      </div>
                      {actFeedback[actId] && (
                        <p
                          className={`text-sm ${actFeedback[actId].status === "success" ? "text-green-600" : "text-red-600"}`}
                        >
                          ...
                          <div key={clue.id} className="rounded-lg border p-4 bg-white/90 shadow-xs space-y-3">
                            ...
                            {!isRevealed ? (
                              <div className="space-y-2">
                                ...
                                {clueFeedback[clueId] && (
                                  <p
                                    className={`text-xs ${clueFeedback[clueId].status === "success" ? "text-green-600" : "text-red-600"}`}
                                  >
      ...
      <Card className="border border-black/10">
        <CardHeader>
          ...
          <CardDescription>
            Entrega una versión corta de tus sospechas y cómo cada pista te llevó hasta ahí.
            {!allActsCompleted && " Completa todos los actos para desbloquear el informe."}
          </CardDescription>
        </CardHeader>
